...
function createLambdaNodeJS(camundaRestEndpoint, userTasks, language) {
...
function parseTaskDescription(taskDescription) {
    // takes e.g. "Prepare \${order} and \${task}" and extracts \${order} and \${task}
    const camundaVariables = taskDescription.match(/\\$\{\\w+\\}/g);
...
const LaunchRequestHandler = {
    ...
    handle(handlerInput) {
        const speakOutput = 'Hey there, you can ask for tasks or complete them. What do you want to do?'; 
        return handlerInput.responseBuilder
            .speak(speakOutput)
            .reprompt(speakOutput)
            .getResponse();
...
const NewTaskIntentHandler = {
    ...
    async handle(handlerInput){
        ...
        let speakOutput = 'There are no open tasks.';

        if (allTasks.length > 0) {
            const task = allTasks[0]
            const taskId = task.id;
            speakOutput = \`Task \${taskId}: \${task.name}\`;
            if (task.description !== null){
                speakOutput = \`Task \${taskId}: \${task.name}. \${task.description}\`;
            }
            try {
                const description = parseTaskDescription(task.description);
                speakOutput = \`Task \${taskId}: \${task.name}. \${description}\`;
            } catch(error) {
                console.log(\`parse task details for \${taskId} failed \`, error);
            }
            try {
...
const IdNotGivenCompleteTaskIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        let speakOutput = 'No task is assigned to you.';
        ...
        if(allAssignedTasks.length > 1) {
            speakOutput = \`Complete Task \${allAssignedTasks[0].id}\`;

            let i;
            for (i = 1; i < allAssignedTasks.length; i++) {
                if (i === allAssignedTasks.length - 1) {
                    speakOutput = speakOutput + \` or \${allAssignedTasks[i].id}?\`;
                } else {
                    speakOutput = speakOutput + \`, \${allAssignedTasks[i].id}\`;
                }
            }
...
const CompletedCompleteTaskIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        let speakOutput = 'This task does not exist.'
        ...
        ${completeTaskWithVars}if (assignedTask.assignee === 'ALEXA') {
            try {
                const completeTask = await axios.post(\`\${camundaRestEndpoint}/task/\${taskId}/complete\`, {});
                speakOutput = \`Task \${taskId} completed. What do you want to do next?\`
            } catch (error) {
                console.log(\`Complete task for taskId \${taskId} failed\`, error)
            }
    ...
    async handle(handlerInput) {
        ...
        if (done) {
            try {
                ...
                const completeTask = await axios.post(\`\${camundaRestEndpoint}/task/\${attributes.taskId}/complete\`, postBody);
                speakOutput = \`Task \${attributes.taskId} completed. What do you want to do next?\`
            } catch (error) {
                console.log(\`Complete task for taskId \${attributes.taskId} failed\`, error)
            }
        }
...
const IdNotGivenTaskDetailsIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        let speakOutput = 'There are no tasks assigned to you.';
        ...
        if(allAssignedTasks.length > 1) {
            speakOutput = \`Task \${allAssignedTasks[0].id}\`;
...
const CompletedTaskDetailsIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        let speakOutput = 'I don't know this task.'
        ...
        if (assignedTask.assignee === 'ALEXA') {
            const taskId = assignedTask.id;
            speakOutput = \`Task \${taskId}: \${assignedTask.name}\`;
            if (assignedTask.description !== null){
                speakOutput = \`Task \${taskId}: \${assignedTask.name}. \${assignedTask.description}\`;
            }
            try {
                const description = parseTaskDescription(assignedTask.description);
                speakOutput = \`Task \${taskId}: \${assignedTask.name}. \${description} Can I do something else for you?\`;
            } catch(error) {
                console.log(\`parse task details for \${taskId} failed \`, error);
            }
        }
...
const HelpIntentHandler = {
    ...
    handle(handlerInput) {
        const attributes = handlerInput.attributesManager.getSessionAttributes();
        attributes.lastIntent = Alexa.getIntentName(handlerInput.requestEnvelope);
        const speakOutput = 'You can claim a task, complete a task or get some details about a task. What do you want to do?';
        handlerInput.attributesManager.setSessionAttributes(attributes);
        return handlerInput.responseBuilder
            .speak(speakOutput)
            .reprompt(speakOutput)
